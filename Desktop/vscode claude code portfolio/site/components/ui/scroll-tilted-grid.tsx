...
export function Tile({ src, side, config, onClick }: { src: string; side: Side; config: TileConfig; onClick?: () => void }) {
  ...
  if (reduce) {
    return (
      <figure ref={ref} className="relative z-10 m-0" onClick={onClick} style={{ cursor: onClick ? "pointer" : undefined }}>
  ...
  return (
    <motion.figure ref={ref} className="relative z-10 m-0" style={{ perspective, willChange: "transform", cursor: onClick ? "pointer" : undefined }} onClick={onClick}>
      <motion.div
